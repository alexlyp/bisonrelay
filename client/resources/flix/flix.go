...
const (
	...
	showTmplFile        = "show.tmpl"
	movieTmplFile       = "movie.tmpl"
...
// Store is a flix instance. A flix can render a front page
// (index) and individual show/movie pages.
type Store struct {
	cfg         Config
	c           *client.Client
	log         slog.Logger
...
func (s *Store) reloadStore() error {
	...
	for _, dir := range dirs {
		...
		for _, filename := range filenames {
			...
			s.log.Debugf("Reloading template %s (name %s)", filename, filepath.Base(filename))
	...
	// Load Shows.
	showsDir := filepath.Join(s.root, showsDir)
	showFiles, err := os.ReadDir(showsDir)
	if err != nil {
		return fmt.Errorf("unable to list show files: %v", err)
	}

	for _, showFile := range showFiles {
		if filepath.Ext(showFile.Name()) != ".toml" {
			continue
		}
		fname := filepath.Join(showsDir, showFile.Name())
		var showFile showsFile
		f, err := os.Open(fname)
		if err != nil {
			return fmt.Errorf("unable to load show file %s: %v",
				fname, err)
		}
		dec := toml.NewDecoder(f)
		err = dec.Decode(&showFile)
		_ = f.Close()
		if err != nil {
			return fmt.Errorf("unable to decode product file %s: %v",
				fname, err)
		}
		for _, show := range showFile.Shows {
			if show.Disabled {
				continue
			}

			if _, ok := shows[show.SKU]; ok {
	...
	for _, movieFile := range movieFiles {
		if filepath.Ext(movieFile.Name()) != ".toml" {
			continue
		}
		fname := filepath.Join(movieDirs, movieFile.Name())
		var movieFile moviesFile
		f, err := os.Open(fname)
		if err != nil {
			return fmt.Errorf("unable to load show file %s: %v",
				fname, err)
...
func (s *Store) Fulfill(ctx context.Context, uid clientintf.UserID,
	...
	case len(request.Path) == 2 && request.Path[0] == "show":
		return s.handleShow(ctx, uid, request)
	case len(request.Path) == 2 && request.Path[0] == "movie":
		return s.handleMovie(ctx, uid, request)
...
func (s *Store) reloadFSWatchers(watcher *fsnotify.Watcher) {
	...
	if err := watcher.Add(filepath.Join(s.root, showsDir)); err != nil {
		s.log.Warnf("Unable to watch shows dir: %v", err)
	}
...
func (s *Store) invoiceSettled(ctx context.Context, order *Order) {
	...
	for _, item := range order.Cart.Items {
		...
		// Relative paths are set to be from the root of the flix.
