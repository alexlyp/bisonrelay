...
func (s *Store) handleBundle(ctx context.Context, uid clientintf.UserID,
	request *rpc.RMFetchResource) (*rpc.RMFetchResourceReply, error) {
	...
	bundle := resources.BundledResource{
		Bundle: rpc.RMResourceBundle{
			Resources: map[string]rpc.RMFetchResourceReply{
				p2s(request.Path): {Status: 200, Data: w.Bytes()},
			},
		},
	}
	for path, movie := range tmplCtx.Movies {
		w := &bytes.Buffer{}
		err := s.tmpl.ExecuteTemplate(w, movieTmplFile, movie)
		if err != nil {
			return nil, fmt.Errorf("unable to execute movie template: %v", err)
		}

		bundle.Bundle.Resources["movie/"+path] =
	...
	for path, show := range tmplCtx.Shows {
		w := &bytes.Buffer{}
		err := s.tmpl.ExecuteTemplate(w, showTmplFile, show)
		if err != nil {
			return nil, fmt.Errorf("unable to execute show template: %v", err)
		}

		bundle.Bundle.Resources["show/"+path] =
			rpc.RMFetchResourceReply{
				Data:   w.Bytes(),
				Status: rpc.ResourceStatusOk,
			}
	...
	res, err := bundle.Fulfill(ctx, uid, request)
	if err != nil {
		return nil, err
	}
...
func (s *Store) handleShow(ctx context.Context, uid clientintf.UserID,
	request *rpc.RMFetchResource) (*rpc.RMFetchResourceReply, error) {

	s.mtx.Lock()
	show := s.shows[request.Path[1]]
	s.mtx.Unlock()
...
func (s *Store) handleMovie(ctx context.Context, uid clientintf.UserID,
	request *rpc.RMFetchResource) (*rpc.RMFetchResourceReply, error) {

	s.mtx.Lock()
	movie := s.movies[request.Path[1]]
	s.mtx.Unlock()
...
func (s *Store) handleAddToCart(ctx context.Context, uid clientintf.UserID,
	...
	content, ok := s.shows[formData.SKU]
	if !ok {
		content, ok = s.movies[formData.SKU]
		return nil, fmt.Errorf("show/movie does not exist")
	}
	...
	for _, item := range cart.Items {
		if item.Content.SKU == content.SKU {
...
func (s *Store) PurchaseOrderRequest(ctx context.Context, uid clientintf.UserID, po rpc.RMPurchaseOrderRequest) error {
	...
	for i := range po.Items {
		prod, ok := s.shows[po.Items[i].SKU]
		if !ok {
			prod, ok = s.movies[po.Items[i].SKU]
			if !ok {
	...
	for _, item := range order.Cart.Items {
		...
		wpm("  SKU %s - %s - %d units - $%.2f/item - $%.2f\n",
			item.Content.SKU, item.Content.Title,
			item.Quantity, item.Content.Price,
...
func (s *Store) handlePlaceOrder(ctx context.Context, uid clientintf.UserID,
	...
	// Verify the items
	for _, item := range cart.Items {
		prod, ok := s.shows[item.Content.SKU]
		if !ok {
			return &rpc.RMFetchResourceReply{
				Status: rpc.ResourceStatusBadRequest,
				Data:   []byte(fmt.Sprintf("SKU %q does not exist", item.Content.SKU)),
			}, nil
		}
		// If a Content requires shipping, ensure a shipping address
		// was sent.
	...
	case s.cfg.ExchangeRateProvider == nil:
		s.log.Warnf("No exchange rate provider setup in flix config")
	...
	if err != nil {
		return nil, fmt.Errorf("unable to execute Content template: %v", err)
	}
	return &rpc.RMFetchResourceReply{
		Data:   w.Bytes(),
		Status: rpc.ResourceStatusOk,
